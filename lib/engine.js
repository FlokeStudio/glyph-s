...
import { embedTextSync, embeddingBoost } from './embeddings.js';
...
export function rankSearchItems(items, q, opts = {}) {
  ...
  // Scan the full corpus — maxCandidates caps the scored top-K pool, not the index prefix.
  const candidates = [];
  ...
  if (settings.semanticEmbeddings && scored.length && String(q || '').trim()) {
    ...
    for (const row of scored) {
      const bag = textBagForItem(row.it).slice(0, 512);
      row.score += embeddingBoost(queryVec, embedTextSync(bag));
    }
  }

  // maxCandidates bounds the scored pool size; limit takes the final top-K from that pool.
  const scoredPool = collectTopK(scored, Math.min(profileCfg.maxCandidates, scored.length || limit));
  const out = scoredPool.slice(0, limit);
