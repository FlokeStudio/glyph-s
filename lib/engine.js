...
async function applySemanticBoostAsync(scored, q, settings, indexRows) {
  ...
  for (let i = 0; i < scored.length; i++) {
    ...
    const row = entry.row || byIt?.get(entry.it) || null;
    const cached = indexEmbeddingFor(row, entry.it);
    // Prefer warmed/non-hash vectors when a model (or custom embedder) is configured.
    if (cached && !(settings.embeddingModelPath || settings.embedder)) {
      itemVecs[i] = cached;
      continue;
    }
    if (cached && row?.embeddingSource === 'onnx') {
      itemVecs[i] = cached;
      continue;
    }
    missingSlots.push(i);
  ...
  if (missingTexts.length) {
    const filled = (await embedTexts(missingTexts, embedCfg)) || missingTexts.map((t) => embedTextSync(t));
...
export async function warmIndexEmbeddings(index, opts = {}) {
  ...
  const source = opts.embedder || opts.embeddingModelPath || opts.modelPath ? 'onnx' : 'hash';
  for (let i = 0; i < index.items.length; i++) {
    if (vectors[i]) {
      index.items[i].embedding = vectors[i];
      index.items[i].embeddingSource = source;
    }
  }
  return index;
}
