...
function collectScoredCandidates(items, q, ctx) {
  const { filters, tokens, indexRows, sourceLen, profileCfg, settings } = ctx;
  const candidates = [];
...
async function applySemanticBoostAsync(scored, q, settings, indexRows) {
  ...
  const queryVec = queryVecs?.[0] ?? embedTextSync(qText);
  ...
  if (missingTexts.length) {
    const itemMeta = { source: querySource };
    const filled =
      (await embedTexts(missingTexts, { ...embedCfg, resultMeta: itemMeta })) ||
      missingTexts.map((t) => embedTextSync(t));
...
export function rankSearchItems(items, q, opts = {}) {
  ...
  const { settings, limit, indexRows, sourceLen } = ctx;
